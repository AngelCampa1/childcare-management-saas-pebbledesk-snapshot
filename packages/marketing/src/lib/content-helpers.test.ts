...
describe("shouldIndexContent", () => {
	it("returns false only for entries explicitly marked noindex", () => {
		expect(shouldIndexContent({ data: { noindex: true } })).toBe(false);
		expect(shouldIndexContent({ data: { noindex: false } })).toBe(true);
		expect(shouldIndexContent({ data: {} })).toBe(true);
	});
});
