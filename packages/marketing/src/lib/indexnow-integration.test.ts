...
describe("indexNowIntegration", () => {
	...
	async function invokeHook(overrideDir?: URL) {
		...
		const hook = integration.hooks["astro:build:done"] as NonNullable<
			(typeof integration.hooks)["astro:build:done"]
		>;
		await hook({
			dir: overrideDir ?? dir,
			logger,
	...
	it("has the correct integration name", () => {
		const integration = indexNowIntegration();
		expect(integration.name).toBe("@pebbledesk/indexnow");
	});
	...
	it("skips submission unless INDEXNOW_SUBMIT is explicitly enabled", async () => {
		...
		expect(logger.info).toHaveBeenCalledWith(
			"IndexNow: skipping submission because INDEXNOW_SUBMIT is not enabled",
		);
		expect(submitSpy).not.toHaveBeenCalled();
	});
