...
describe("composeMetaTitle", () => {
	it("appends the site name when the composed title stays within the limit", () => {
		expect(composeMetaTitle("Pricing", "PebbleDesk")).toBe("Pricing | PebbleDesk");
	});

	it("keeps the page title unbranded when the brand suffix would exceed the limit", () => {
		const pageTitle = "Childcare Staff Scheduling Software for Licensed Centers";

		expect(composeMetaTitle(pageTitle, "PebbleDesk")).toBe(pageTitle);
	});

	it("truncates the unbranded page title only as a fallback", () => {
		const pageTitle =
			"Arizona Childcare Software for Licensed Centers Managing Subsidy Billing and Audits";
		const result = composeMetaTitle(pageTitle, "PebbleDesk");

		expect(result).not.toContain("PebbleDesk");
		expect(result.length).toBeLessThanOrEqual(60);
		expect(result).toBe(truncateMetaTitle(pageTitle));
	});

	it("does not append the site name twice", () => {
		expect(composeMetaTitle("Pricing | PebbleDesk", "PebbleDesk")).toBe("Pricing | PebbleDesk");
	});
});
...
describe("resolveSchemaAuthor", () => {
	it("converts a relative author url to an absolute site url for schema", () => {
		expect(resolveSchemaAuthor({ name: "Angel", url: "/about/" }, "pebbledesk.app")).toEqual({
			name: "Angel",
			url: "https://pebbledesk.app/about/",
		});
	});

	it("keeps absolute author urls unchanged", () => {
		expect(resolveSchemaAuthor({ name: "Jane", url: "https://jane.dev" }, "example.com")).toEqual({
			name: "Jane",
			url: "https://jane.dev",
		});
	});

	it("returns undefined when no author is configured", () => {
		expect(resolveSchemaAuthor(undefined, "example.com")).toBeUndefined();
	});
});
