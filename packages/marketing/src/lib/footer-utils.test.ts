...
describe("buildFooterEmailCaptureProps", () => {
	it("returns undefined when footer.emailCapture is not set", () => {
		...
		expect(buildFooterEmailCaptureProps(config, "/resources/guides/example")).toBeUndefined();
	});
	...
	it("returns CTA props sourced from the BOFU funnel target when footer.emailCapture is set", () => {
		...
		const result = buildFooterEmailCaptureProps(config, "/compare/pricing/example");
		...
		expect(result).toEqual({
			...
			ctaText: "View launch pricing",
			ctaTarget:
				"https://my.pebbledesk.app/signup?plan=center_starter&source=%2Fcompare%2Fpricing%2Fexample",
		});
	});
	...
	it("returns CTA props without leaking email-capture copy when copy.emailCapture is set", () => {
		...
		const result = buildFooterEmailCaptureProps(config, "/privacy");
		...
		expect(result.ctaTarget).toBe("https://my.pebbledesk.app/signup?source=%2Fprivacy");
	...
	it("uses the configured BOFU CTA when a site targets a non-default fake-door destination", () => {
		...
		const result = buildFooterEmailCaptureProps(config, "/free/checklist");
		if (!result || result.mode !== "cta") {
			throw new Error("Expected CTA footer props");
		}
		expect(result.ctaText).toBe("See accessibility pricing");
		expect(result.ctaTarget).toBe("https://my.pebbledesk.app/signup?source=%2Ffree%2Fchecklist");
	});
});
