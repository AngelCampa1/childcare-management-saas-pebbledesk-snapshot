...
const siteConfig = {
	...
	copy: {
		emailCapture: {
			...
			whatHappensNext: "We will email your access link and setup steps.",
...
describe("buildPublicSignupFlowConfig", () => {
	...
	it("uses a neutral default access-link message when email capture copy is absent", () => {
		const configWithoutEmailCaptureCopy = {
			...siteConfig,
			copy: {
		...
		const result = buildPublicSignupFlowConfig(configWithoutEmailCaptureCopy);
		const { whatHappensNext } = result;

		expect(whatHappensNext).toBe("We will email your access link and setup steps.");
		expect(whatHappensNext?.toLowerCase()).not.toContain("follow-up");
		expect(whatHappensNext?.toLowerCase()).not.toContain("sequence");
		expect(whatHappensNext?.toLowerCase()).not.toContain("unsubscribe");
		expect(whatHappensNext?.toLowerCase()).not.toContain("spam");
	});
});
