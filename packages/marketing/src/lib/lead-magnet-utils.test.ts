...
import { splitContentAtGate, stripLeadingHeadingOne } from "./lead-magnet-utils";
...
describe("stripLeadingHeadingOne", () => {
	it("removes a leading markdown-rendered h1", () => {
		expect(stripLeadingHeadingOne('<h1 id="title">Title</h1><p>Intro.</p>')).toBe("<p>Intro.</p>");
	});

	it("preserves non-leading h1 content", () => {
		const html = "<p>Intro.</p><h1>Embedded title</h1><p>Body.</p>";

		expect(stripLeadingHeadingOne(html)).toBe(html);
	});
});
