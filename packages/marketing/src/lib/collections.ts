import type { BuyerStage, ContentItem } from "../types";
...
>(
	...
	return entries.map((entry) => {
		...
		return {
			...
			...(entry.data.lastReviewed !== undefined && {
			...
			...(entry.data.canonicalHref !== undefined && {
				canonical: entry.data.canonicalHref,
			}),
			...(entry.data.redirectFrom !== undefined &&
				entry.data.redirectFrom.length > 0 && {
					redirectFrom: entry.data.redirectFrom,
				}),
			...(entry.data.summaryFacts !== undefined &&
				entry.data.summaryFacts.length > 0 && {
					summaryFacts: entry.data.summaryFacts,
				}),
