...
describe("buildLlmsTxtSections", () => {
	...
	it("keeps file-extension paths exact for machine-readable assets", () => {
		const sections = buildLlmsTxtSections("https://example.com", [
			{
				heading: "AI Data",
				entries: [
		...
		expect(sections[0]?.items[0]?.url).toBe("https://example.com/ai/manifest.json");
	});

	it("requires title and description resolvers for non-standard entries", () => {
		expect(() =>
			buildLlmsTxtSections("https://example.com", [
				{
					heading: "Broken",
		...
		expect(() =>
			buildLlmsTxtSections("https://example.com", [
				{
					heading: "Broken",
					entries: [{ data: { title: "Missing description" } }],
					path: "/missing-description",
					title: (entry) => entry.data.title,
				},
			]),
		).toThrow(/data\.description/);
	});
});
