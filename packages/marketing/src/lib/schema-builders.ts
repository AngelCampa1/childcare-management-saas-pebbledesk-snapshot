...
function isExtensionlessPageUrl(url: URL): boolean {
	const lastSegment = url.pathname.split("/").filter(Boolean).at(-1) ?? "";
	return !lastSegment.includes(".");
}

function normalizeSchemaUrl(url: string, siteUrl?: string): string {
	if (/^https?:\/\//.test(url) && siteUrl === undefined) {
		return url;
	}

	const absoluteUrl =
		/^https?:\/\//.test(url) || siteUrl === undefined ? url : joinUrl(siteUrl, url);
...
function schemaSiteUrlFromPageUrl(url: string): string {
	try {
...
export function buildBreadcrumbSchema(items: BreadcrumbItemInput[], siteUrl: string): JsonLdSchema {
	return {
		...
		itemListElement: items.map((item, index) => ({
			...
			position: index + 1,
			name: item.label,
			item: { "@id": normalizeSchemaUrl(item.href, siteUrl), name: item.label },
		})),
	};
}
...
function resolveItemListUrl(url: string | undefined, siteUrl?: string): string | undefined {
	if (url === undefined) {
		return undefined;
	}

	return normalizeSchemaUrl(url, siteUrl);
}

export function buildItemListSchema(items: ItemListItemInput[], siteUrl?: string): JsonLdSchema {
	return {
		"@context": "https://schema.org",
		"@type": "ItemList",
		itemListElement: items.map((item, index) => ({
			...
			...(resolveItemListUrl(item.url, siteUrl) !== undefined && {
				url: resolveItemListUrl(item.url, siteUrl),
			}),
		})),
	};
}
...
export function buildWebPageSchema(opts: WebPageSchemaOpts): JsonLdSchema {
	const { id, url, name, description, mainEntityId, breadcrumbId } = opts;
	return {
		"@context": "https://schema.org",
		"@type": "WebPage",
		"@id": id,
...
export function buildCollectionPageSchema(opts: CollectionPageSchemaOpts): JsonLdSchema {
	const { name, description, url, items, siteUrl } = opts;
	const canonicalSiteUrl = siteUrl ?? schemaSiteUrlFromPageUrl(url);
	return {
		"@context": "https://schema.org",
		"@type": "CollectionPage",
		name,
		description,
		url: normalizeSchemaUrl(url, canonicalSiteUrl),
		...(items &&
			items.length > 0 && {
				mainEntity: {
					"@type": "ItemList",
					itemListElement: items.map((item, index) => ({
						...
						...(item.url !== undefined && {
							url: normalizeSchemaUrl(item.url, canonicalSiteUrl),
						}),
					})),
				},
			}),
