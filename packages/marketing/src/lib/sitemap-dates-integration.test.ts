...
describe("rewriteSitemapDates", () => {
	it("does not rewrite unrelated canonical paths that merely share a slug suffix", () => {
		const sitemapXml = `<urlset><url><loc>https://example.com/free/servicetitan</loc><lastmod>2026-03-27T00:00:00.000Z</lastmod></url></urlset>`;
		const result = rewriteSitemapDates(sitemapXml, {
			"/compare/alternatives/servicetitan": "2026-01-01",
		});

		expect(result).toBe(sitemapXml);
	});

	it("rewrites lastmod by exact canonical path", () => {
		const sitemapXml = `<urlset><url><loc>https://example.com/compare/alternatives/servicetitan/</loc><lastmod>2026-03-27T00:00:00.000Z</lastmod></url></urlset>`;
		const result = rewriteSitemapDates(sitemapXml, {
			"/compare/alternatives/servicetitan": "2026-01-01",
		});
	...
	it("inserts lastmod after loc when a matching canonical path has no lastmod", () => {
		const sitemapXml = `<urlset><url><loc>https://example.com/compare/alternatives/servicetitan/</loc><changefreq>monthly</changefreq><priority>0.9</priority></url></urlset>`;
		const result = rewriteSitemapDates(sitemapXml, {
			"/compare/alternatives/servicetitan": "2026-01-01",
		});

		expect(result).toContain(
			"<loc>https://example.com/compare/alternatives/servicetitan/</loc><lastmod>2026-01-01</lastmod><changefreq>monthly</changefreq>",
		);
	});
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url>
	...
	it("replaces lastmod for a matching canonical path", () => {
		const result = rewriteSitemapDates(sampleXml, {
			"/compare/alternatives/servicetitan": "2026-03-15",
		});
	...
	it("replaces lastmod for multiple matching canonical paths", () => {
		const result = rewriteSitemapDates(sampleXml, {
			"/compare/alternatives/servicetitan": "2026-03-15",
			"/resources/guides/how-to-dispatch": "2026-02-20",
		});
		expect(result).toContain("<lastmod>2026-03-15</lastmod>");
  ...
  <sitemap>
	...
	it("handles <lastmod> with different whitespace patterns", () => {
		...
		const result = rewriteSitemapDates(xml, {
			"/resources/guides/my-guide": "2026-01-05",
		});
...
<urlset>
  ...
  <url>
    ...
    <lastmod>2026-03-27T00:00:00.000Z</lastmod>
		...
		const slugMap = {
			"/resources/guides/perimenopause-anxiety": "2026-01-01",
...
describe("sitemapDatesIntegration", () => {
	...
	it("returns an integration with the correct name", () => {
		const integration = sitemapDatesIntegration({
			resolveCanonicalPath: (slug) =>
				slug === "alternatives/servicetitan" ? "/compare/alternatives/servicetitan" : `/${slug}`,
		});
	...
	it("warns and returns early when src/content directory does not exist", async () => {
		...
		expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining("src/content not found"));
	});
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  <url>
	...
	it("inserts lastmod in sitemap XML with no existing lastmod tags", async () => {
		...
		expect(mockWriteFileSync).toHaveBeenCalledWith(
			join("\\fake\\dist", "sitemap-0.xml"),
			"<urlset><url><loc>https://example.com/alternatives/servicetitan</loc><lastmod>2026-01-20</lastmod></url></urlset>",
			"utf-8",
		);
		expect(logger.info).toHaveBeenCalledWith(expect.stringContaining("rewrote 1 lastmod entries"));
	});
