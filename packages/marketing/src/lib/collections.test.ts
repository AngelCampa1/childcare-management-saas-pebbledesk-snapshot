...
describe("mapToContentItems", () => {
	...
	it("includes canonical, redirectFrom, lastReviewed, and summaryFacts when entry has them", () => {
		const items = mapToContentItems(
			[
				{
					slug: "test-entry",
					data: {
						...
						publishedAt: "2026-01-01",
						updatedAt: "2026-02-01",
						canonicalHref: "/articles/canonical-entry",
						redirectFrom: ["/articles/old-entry"],
						lastReviewed: "2026-04-10",
						summaryFacts: ["PebbleDesk keeps subsidy claims and attendance aligned."],
					},
				},
			],
			(e) => `/articles/${e.slug}`,
		...
		expect(items[0].canonical).toBe("/articles/canonical-entry");
		expect(items[0].redirectFrom).toEqual(["/articles/old-entry"]);
		expect(items[0].lastReviewed).toBe("2026-04-10");
		expect(items[0].summaryFacts).toEqual([
			"PebbleDesk keeps subsidy claims and attendance aligned.",
		]);
	});
});
