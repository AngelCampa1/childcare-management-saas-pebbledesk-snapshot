...
import { buildGraph, buildPublisherGraphNodes, refId, withId } from "./schema-graph";
...
describe("buildPublisherGraphNodes", () => {
	it("builds resolvable Organization and WebSite nodes without telephone fields", () => {
		const nodes = buildPublisherGraphNodes({
			siteUrl: "https://pebbledesk.app",
			name: "PebbleDesk",
			description: "Childcare platform",
		});

		expect(nodes).toEqual([
			expect.objectContaining({
				"@type": "Organization",
				"@id": "https://pebbledesk.app/#organization",
				name: "PebbleDesk",
				url: "https://pebbledesk.app",
			}),
			expect.objectContaining({
				"@type": "WebSite",
				"@id": "https://pebbledesk.app/#website",
				name: "PebbleDesk",
				url: "https://pebbledesk.app",
				publisher: { "@id": "https://pebbledesk.app/#organization" },
			}),
		]);
		expect(JSON.stringify(nodes)).not.toContain("telephone");
	});

	it("normalizes a trailing site URL before building publisher IDs", () => {
		const nodes = buildPublisherGraphNodes({
			siteUrl: "https://pebbledesk.app/",
			name: "PebbleDesk",
		});

		expect(nodes[0]?.["@id"]).toBe("https://pebbledesk.app/#organization");
		expect(nodes[0]?.url).toBe("https://pebbledesk.app");
		expect(nodes[1]?.["@id"]).toBe("https://pebbledesk.app/#website");
	});
});
