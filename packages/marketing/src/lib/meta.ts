import type { BuyerStage, SiteAuthor, SiteConfig } from "../types";
...
export function composeMetaTitle(title: string, siteName: string, maxLen = 60): string {
	const trimmedTitle = title.trim();
	const trimmedSiteName = siteName.trim();
	if (!trimmedTitle || !trimmedSiteName) {
		return truncateMetaTitle(trimmedTitle || trimmedSiteName, maxLen);
	}

	const suffix = ` | ${trimmedSiteName}`;
	if (trimmedTitle.endsWith(suffix)) {
		return truncateMetaTitle(trimmedTitle, maxLen);
	}

	const composed = `${trimmedTitle}${suffix}`;
...
export function resolveSchemaAuthor(
	author: SiteAuthor | undefined,
	domain: string,
): SiteAuthor | undefined {
	if (!author?.url?.startsWith("/")) return author;

	return {
		...author,
		url: `https://${domain}${author.url}`,
	};
}
