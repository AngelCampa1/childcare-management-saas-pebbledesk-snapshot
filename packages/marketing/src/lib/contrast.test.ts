...
import { contrastRatio, meetsAA, meetsAAA, relativeLuminance } from "./contrast.js";
import { darkModeVariant, generateScale, generateThemeCSS } from "./generate-theme-css.js";
...
function getDarkBlock(css: string): string {
	const match = css.match(/@media[^{]*\{[^{]*:root:not\(\.light\)\s*\{([\s\S]*?)\n {2}\}/);
	expect(match, "Expected generated CSS to include a dark-mode block").not.toBeNull();
...
describe("dark mode contrast audit", () => {
	...
	it.each(testCases)("[$siteName] $label: fg=$fg bg=$bg must meet $minRatio:1", ({
		siteName,
		label,
		fg,
		bg,
...
describe("Horiva generated dark mode contrast", () => {
	const horivaTheme = {
		primary: "#6B2D8B",
		accent: "#C4622D",
		surface: "#FDF8F4",
	...
	it("keeps generated dark text, muted text, and CTA colors above AA contrast", () => {
		const css = generateThemeCSS(horivaTheme);
		const darkBlock = getDarkBlock(css);

		const darkSurface = getTokenValue(darkBlock, "--site-surface");
		const darkText = getTokenValue(darkBlock, "--site-text");
		const darkMuted = getTokenValue(darkBlock, "--site-muted");
		const darkAccent = getTokenValue(darkBlock, "--site-accent");
