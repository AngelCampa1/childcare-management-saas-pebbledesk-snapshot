...
describe("buildBreadcrumbSchema", () => {
	...
	it("constructs absolute URL by concatenating siteUrl and href", () => {
		...
		expect(items[0].item["@id"]).toBe("https://crewroute.com/guides/dispatching/");
	});
	...
	it("normalizes extensionless same-origin breadcrumb URLs to trailing slashes", () => {
		const result = buildBreadcrumbSchema(
			[
				{ label: "Compare", href: "/compare" },
				{ label: "Guide", href: "/resources/guides/foo" },
				{ label: "Already", href: "/pricing/" },
			],
		...
		expect(items.map((item) => item.item["@id"])).toEqual([
			"https://pebbledesk.app/compare/",
			"https://pebbledesk.app/resources/guides/foo/",
			"https://pebbledesk.app/pricing/",
		]);
	});
});
...
describe("buildWebPageSchema", () => {
	it("connects the page URL, main entity, and breadcrumb list by @id", () => {
		const result = buildWebPageSchema({
			id: "https://pebbledesk.app/resources/guides/test/",
			url: "https://pebbledesk.app/resources/guides/test/",
			name: "Test guide",
			description: "A test guide.",
			mainEntityId: "https://pebbledesk.app/resources/guides/test/#article",
			breadcrumbId: "https://pebbledesk.app/resources/guides/test/#breadcrumb",
		});
		...
		expect(result).toEqual({
			...
			name: "Test guide",
			description: "A test guide.",
			mainEntity: { "@id": "https://pebbledesk.app/resources/guides/test/#article" },
			breadcrumb: { "@id": "https://pebbledesk.app/resources/guides/test/#breadcrumb" },
		});
	});
});
...
describe("buildItemListSchema", () => {
	...
	it("converts relative item urls into absolute urls when siteUrl is provided", () => {
		const result = buildItemListSchema(
			[{ name: "Guide A", url: "/resources/guides/test-guide" }],
			"https://pebbledesk.app/",
		);
		const items = result.itemListElement as { url?: string }[];
		expect(items[0].url).toBe("https://pebbledesk.app/resources/guides/test-guide/");
	});

	it("normalizes extensionless internal item urls to trailing slashes", () => {
		const result = buildItemListSchema(
			[
				{ name: "Compare", url: "/compare" },
				{ name: "Guide", url: "/resources/guides/foo" },
				{ name: "Already", url: "/pricing/" },
			],
			"https://pebbledesk.app",
		);
		const items = result.itemListElement as { url?: string }[];
	...
	it("preserves absolute item urls when siteUrl is provided", () => {
		const result = buildItemListSchema(
			[{ name: "Guide A", url: "https://example.com/test-guide" }],
			"https://pebbledesk.app/",
		);
		const items = result.itemListElement as { url?: string }[];
		expect(items[0].url).toBe("https://example.com/test-guide");
	});
});
...
describe("buildCollectionPageSchema", () => {
	...
	it("normalizes same-site extensionless collection and item URLs to trailing slashes", () => {
		const result = buildCollectionPageSchema({
			name: "Compare Hub",
			description: "Compare childcare software",
			url: "https://pebbledesk.app/compare",
			items: [
				{ name: "Alternatives", url: "https://pebbledesk.app/compare/alternatives" },
				{ name: "ChildPilot", url: "https://pebbledesk.app/compare/alternatives/childpilot" },
				{ name: "Sitemap", url: "https://pebbledesk.app/sitemap-index.xml" },
				{ name: "External", url: "https://example.org/external" },
			],
		}) as Record<string, unknown>;

		expect(result.url).toBe("https://pebbledesk.app/compare/");
		const mainEntity = result.mainEntity as Record<string, unknown>;
		const elements = mainEntity.itemListElement as Record<string, unknown>[];
		expect(elements.map((element) => element.url)).toEqual([
			"https://pebbledesk.app/compare/alternatives/",
			"https://pebbledesk.app/compare/alternatives/childpilot/",
			"https://pebbledesk.app/sitemap-index.xml",
			"https://example.org/external",
		]);
	});

	it("resolves relative collection item URLs from the site origin", () => {
		const result = buildCollectionPageSchema({
			name: "Compare Hub",
			description: "Compare childcare software",
			url: "https://pebbledesk.app/compare",
			items: [{ name: "Alternatives", url: "/compare/alternatives" }],
		}) as Record<string, unknown>;

		const mainEntity = result.mainEntity as Record<string, unknown>;
		const elements = mainEntity.itemListElement as Record<string, unknown>[];
		expect(elements[0].url).toBe("https://pebbledesk.app/compare/alternatives/");
	});
	...
	it("omits description and url keys when they are undefined on an item", () => {
		...
		const mainEntity = result.mainEntity as Record<string, unknown>;
		const elements = mainEntity.itemListElement as Record<string, unknown>[];
		expect(Object.hasOwn(elements[0], "description")).toBe(false);
		expect(Object.hasOwn(elements[0], "url")).toBe(false);
	});
});
