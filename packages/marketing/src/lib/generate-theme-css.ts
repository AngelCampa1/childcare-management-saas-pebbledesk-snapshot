...
function xyzToLinearRgb(x: number, y: number, z: number): { lr: number; lg: number; lb: number } {
	return {
...
function oklabToXyz(L: number, a: number, b: number): { x: number; y: number; z: number } {
...
function oklchToLinearRgb(L: number, C: number, h: number): { lr: number; lg: number; lb: number } {
...
function oklchToHex(L: number, C: number, h: number): string {
	...
	if (!isInSrgbGamut(rgb.lr) || !isInSrgbGamut(rgb.lg) || !isInSrgbGamut(rgb.lb)) {
		let low = 0;
		let high = C;
...
export const SCALE_SUFFIXES = [50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950] as const;
 ...
 */
const DARK_LIGHTNESS_STEPS = [12, 15, 20, 27, 35, 45, 56, 69, 79, 87, 93] as const;

/**
 * Generate an 11-step dark neutral scale for tinted surfaces.
 * Uses the same hue/saturation as the light neutral scale but with
 * dark-mode-appropriate lightness values.
 */
function generateDarkNeutralScale(
	surface: string,
): Record<(typeof SCALE_SUFFIXES)[number], string> {
	const { r, g, b } = hexToRgb(surface);
	const { h, s } = rgbToOklchScaled(r, g, b);
	...
	for (let i = 0; i < SCALE_SUFFIXES.length; i++) {
		const suffix = SCALE_SUFFIXES[i];
		const lightness = DARK_LIGHTNESS_STEPS[i];
		result[suffix] = oklchScaledToHex(h, neutralSat, lightness);
	}
	return result;
}

function deriveDarkDefaults(theme: Theme): {
	surface: string;
	text: string;
	muted: string;
	neutralScale?: Record<(typeof SCALE_SUFFIXES)[number], string>;
} {
	const tintedDarkNeutralScale =
		theme.surface && isTintedSurface(theme.surface)
			? generateDarkNeutralScale(theme.surface)
			: undefined;

	return {
		surface: theme.dark?.surface ?? tintedDarkNeutralScale?.[50] ?? "#0f172a",
		text: theme.dark?.text ?? tintedDarkNeutralScale?.[950] ?? "#f1f5f9",
		muted: theme.dark?.muted ?? tintedDarkNeutralScale?.[700] ?? "#94a3b8",
		neutralScale: tintedDarkNeutralScale,
	};
}

/**
 * Emit dark neutral CSS vars as unindented lines (caller adds indent).
 */
function darkNeutralScaleToLines(scale: Record<(typeof SCALE_SUFFIXES)[number], string>): string[] {
	return SCALE_SUFFIXES.map((step) => `--site-neutral-${step}-dark: ${scale[step]};`);
}
...
function getThemePresentationTokens(
	theme: Theme,
	mode: "light" | "dark" = "light",
): ThemePresentationTokens {
	...
	> = isDark
		? {
				glass: {
					surfaceSecondary: "color-mix(in srgb, var(--site-surface) 88%, white 12%)",
					surfaceElevated: "color-mix(in srgb, var(--site-surface) 94%, white 6%)",
					surfaceSunken: "color-mix(in srgb, var(--site-surface) 90%, black 10%)",
					surfaceGlass: "color-mix(in srgb, var(--site-surface) 78%, transparent)",
					surfaceGlassBorder:
						"color-mix(in srgb, var(--site-accent) 16%, rgba(255, 255, 255, 0.16))",
					sectionHighlightBg: "color-mix(in srgb, var(--site-surface) 82%, var(--site-accent) 18%)",
				},
				flat: {
					surfaceSecondary: "color-mix(in srgb, var(--site-surface) 92%, white 8%)",
					surfaceElevated: "color-mix(in srgb, var(--site-surface) 97%, white 3%)",
					surfaceSunken: "color-mix(in srgb, var(--site-surface) 94%, black 6%)",
					surfaceGlass: "color-mix(in srgb, var(--site-surface) 94%, transparent)",
					surfaceGlassBorder: "color-mix(in srgb, rgba(255, 255, 255, 0.22) 60%, transparent)",
					sectionHighlightBg: "color-mix(in srgb, var(--site-surface) 90%, var(--site-accent) 10%)",
				},
				layered: {
					surfaceSecondary: "color-mix(in srgb, var(--site-surface) 78%, var(--site-primary) 22%)",
					surfaceElevated: "color-mix(in srgb, var(--site-surface) 88%, white 12%)",
					surfaceSunken: "color-mix(in srgb, var(--site-surface) 84%, black 16%)",
					surfaceGlass: "color-mix(in srgb, var(--site-surface) 74%, transparent)",
					surfaceGlassBorder:
						"color-mix(in srgb, var(--site-primary) 26%, rgba(255, 255, 255, 0.14))",
					sectionHighlightBg: "color-mix(in srgb, var(--site-surface) 74%, var(--site-accent) 26%)",
				},
			}
		: {
				glass: {
	...
	> = isDark
		? {
				subtle: {
					shadowCard: "0 2px 8px -2px rgba(0, 0, 0, 0.24), 0 1px 3px -2px rgba(0, 0, 0, 0.18)",
					shadowLg: "0 8px 18px -6px rgba(0, 0, 0, 0.34), 0 3px 8px -5px rgba(0, 0, 0, 0.22)",
					shadowAmbient: "0 16px 28px rgba(0, 0, 0, 0.22)",
				},
				balanced: {
					shadowCard: "0 4px 12px -2px rgba(0, 0, 0, 0.28), 0 2px 6px -2px rgba(0, 0, 0, 0.2)",
					shadowLg: "0 10px 15px -3px rgba(0, 0, 0, 0.4), 0 4px 6px -4px rgba(0, 0, 0, 0.26)",
					shadowAmbient: "0 20px 40px rgba(0, 0, 0, 0.24)",
				},
				strong: {
					shadowCard: "0 10px 24px -10px rgba(0, 0, 0, 0.38), 0 3px 10px -5px rgba(0, 0, 0, 0.26)",
					shadowLg: "0 18px 32px -12px rgba(0, 0, 0, 0.48), 0 6px 12px -6px rgba(0, 0, 0, 0.32)",
					shadowAmbient: "0 28px 56px rgba(0, 0, 0, 0.3)",
				},
			}
		: {
				subtle: {
	...
	const ctaByStyle: Record<NonNullable<Theme["ctaStyle"]>, ThemePresentationTokens["cta"]> = isDark
		? {
				solid: {
					primaryButtonBg: "var(--site-accent)",
					primaryButtonHoverBg: "color-mix(in srgb, var(--site-accent) 88%, white 12%)",
					primaryButtonFg: "var(--site-surface)",
					primaryButtonBorder: "1px solid transparent",
					primaryButtonShadow:
						"0 12px 26px -18px color-mix(in srgb, var(--site-accent) 55%, transparent), 0 4px 12px rgba(0, 0, 0, 0.28)",
					primaryButtonRadius: "9999px",
				},
				soft: {
					primaryButtonBg: "color-mix(in srgb, var(--site-accent) 24%, var(--site-surface) 76%)",
					primaryButtonHoverBg:
						"color-mix(in srgb, var(--site-accent) 34%, var(--site-surface) 66%)",
					primaryButtonFg: "var(--site-text)",
					primaryButtonBorder: "1px solid color-mix(in srgb, var(--site-accent) 28%, transparent)",
					primaryButtonShadow:
						"0 10px 24px -18px color-mix(in srgb, var(--site-accent) 48%, transparent)",
					primaryButtonRadius: "9999px",
				},
				outline: {
					primaryButtonBg: "transparent",
					primaryButtonHoverBg:
						"color-mix(in srgb, var(--site-accent) 14%, var(--site-surface) 86%)",
					primaryButtonFg: "var(--site-accent)",
					primaryButtonBorder: "1px solid color-mix(in srgb, var(--site-accent) 48%, transparent)",
					primaryButtonShadow:
						"0 6px 18px -16px color-mix(in srgb, var(--site-accent) 38%, transparent)",
					primaryButtonRadius: "9999px",
				},
			}
		: {
	...
	const surfaceSecondaryOverride =
		isDark && theme.dark?.surfaceSecondary !== undefined
	...
	return {
		...
		surfaces: {
			...surfaces,
			surfaceSecondary: surfaceSecondaryOverride,
			sectionHighlightBg:
				chromeEmphasis === "subtle"
					? isDark
						? "color-mix(in srgb, var(--site-surface) 90%, var(--site-accent) 10%)"
 ...
 * - Dark mode block covering both `@media` and `:root.dark`
 */
export function generateThemeCSS(theme: SiteConfig["theme"]): string {
	...
	// Dark mode primary/accent variants
	const darkDefaults = deriveDarkDefaults(theme);
	const darkPrimary = theme.dark?.primary ?? darkModeVariant(theme.primary);
	const darkAccent = theme.dark?.accent ?? darkModeVariant(theme.accent);
	const darkSurface = theme.dark?.surface ?? darkDefaults.surface;
	const darkText = theme.dark?.text ?? darkDefaults.text;
	const darkMuted = theme.dark?.muted ?? darkDefaults.muted;
	const darkPrimaryScale = generateScale(darkPrimary);
	const darkAccentScale = generateScale(darkAccent);
	const darkPresentation = getThemePresentationTokens(
		{
			...theme,
			primary: darkPrimary,
			accent: darkAccent,
	...
	// Dark mode primary/accent scale hooks (step 50 and 100 use the darkest
	// generated values; steps 950 and 900 respectively; since in dark mode
	// the "50" slot is used for dark chip/card backgrounds).
	const darkPrimary50 = darkPrimaryScale[950];
	const darkPrimary100 = darkPrimaryScale[900];
	const darkAccent50 = darkAccentScale[950];
	...
	const rootLines = [
		...
		`  --site-surface-frost: ${presentation.surfaces.surfaceGlass};`,
		`  --site-surface-frost-border: ${presentation.surfaces.surfaceGlassBorder};`,
	...
	// Build dark mode vars (no leading indent; indent is added per-block below)
	const darkVarLines: string[] = [
		`--site-primary: ${darkPrimary};`,
		`--site-accent: ${darkAccent};`,
		`--site-surface: ${darkSurface};`,
		`--site-text: ${darkText};`,
		`--site-muted: ${darkMuted};`,
		`--site-surface-secondary-dark: ${darkPresentation.surfaces.surfaceSecondary};`,
		`--site-surface-elevated-dark: ${darkPresentation.surfaces.surfaceElevated};`,
		`--site-surface-sunken-dark: ${darkPresentation.surfaces.surfaceSunken};`,
		`--site-surface-frost-dark: ${darkPresentation.surfaces.surfaceGlass};`,
		`--site-surface-frost-border-dark: ${darkPresentation.surfaces.surfaceGlassBorder};`,
		`--site-section-highlight-bg-dark: ${darkPresentation.surfaces.sectionHighlightBg};`,
		`--site-shadow-card-dark: ${darkPresentation.shadows.shadowCard};`,
		`--site-shadow-lg-dark: ${darkPresentation.shadows.shadowLg};`,
		`--site-shadow-ambient-dark: ${darkPresentation.shadows.shadowAmbient};`,
		`--site-primary-button-bg-dark: ${darkPresentation.cta.primaryButtonBg};`,
		`--site-primary-button-hover-bg-dark: ${darkPresentation.cta.primaryButtonHoverBg};`,
		`--site-primary-button-fg-dark: ${darkPresentation.cta.primaryButtonFg};`,
		`--site-primary-button-border-dark: ${darkPresentation.cta.primaryButtonBorder};`,
		`--site-primary-button-shadow-dark: ${darkPresentation.cta.primaryButtonShadow};`,
		`--site-primary-50-dark: ${darkPrimary50};`,
		`--site-primary-100-dark: ${darkPrimary100};`,
		`--site-accent-50-dark: ${darkAccent50};`,
	];

	if (theme.dark?.surface !== undefined) {
		darkVarLines.push(`--site-surface-dark: ${theme.dark.surface};`);
	}
	if (theme.dark?.surfaceSecondary !== undefined) {
		darkVarLines.push(`--site-surface-secondary: ${theme.dark.surfaceSecondary};`);
	}
	if (theme.dark?.text !== undefined) {
		darkVarLines.push(`--site-text-dark: ${theme.dark.text};`);
	}
	if (theme.dark?.muted !== undefined) {
		darkVarLines.push(`--site-muted-dark: ${theme.dark.muted};`);
	}

	// Dark neutral scale; only emit when surface is tinted
	if (darkDefaults.neutralScale) {
		darkVarLines.push(...darkNeutralScaleToLines(darkDefaults.neutralScale));
	}

	const indented = darkVarLines.map((l) => `    ${l}`).join("\n");
	...
	return [`:root {`, rootLines, `}`, ``, darkBlock, ``].join("\n");
}
