...
describe("initMobileNav", () => {
	...
	it("closes the nav when the dedicated close button is clicked", () => {
		const { details, trigger } = buildMobileNav();
		const closeButton = document.createElement("button");
		closeButton.type = "button";
		closeButton.dataset.mobileNavClose = "";
		closeButton.setAttribute("aria-label", "Close navigation menu");
		details.querySelector("nav[aria-label='Mobile navigation']")?.prepend(closeButton);
