...
describe("setupScrollDepthTracking", () => {
	it("fires at each threshold as scroll progresses", () => {
		...
		setScrollProps(2000, 0, 500);
		fireScroll();
		expect(mockTrackEvent).toHaveBeenCalledWith(ANALYTICS_EVENTS.scrollDepthReached, {
			threshold: 25,
			page_path: "/test-page",
		});
	...
	it("does not re-fire past same threshold", () => {
		...
		const calls25 = mockTrackEvent.mock.calls.filter(
			(c) =>
				c[0] === ANALYTICS_EVENTS.scrollDepthReached &&
	...
	it("page_path is included in properties", () => {
		...
		expect(mockTrackEvent).toHaveBeenCalledWith(
			ANALYTICS_EVENTS.scrollDepthReached,
...
describe("setupSectionVisibilityTracking", () => {
	it("fires section_viewed when element intersects at 30%+", () => {
		...
		const observer = MockIntersectionObserver.instances[0] as NonNullable<
			(typeof MockIntersectionObserver.instances)[0]
		>;
...
describe("setupFaqExpansionTracking", () => {
    ...
    `;
		...
		(allDetails[1] as HTMLDetailsElement).open = true;
...
describe("setupCtaClickTracking", () => {
	it("fires CTA click event when clicking a[href^='#']", () => {
		document.body.innerHTML = `
    ...
    `;
		...
		expect(mockTrackEvent).toHaveBeenCalledWith(ANALYTICS_EVENTS.ctaClicked, {
			href: "#pricing",
			section: "pricing",
			page_path: "/test-page",
		});
	...
	it("fires CTA click event when clicking [data-cta-button]", () => {
		document.body.innerHTML = `
	...
	it("fires CTA click event when clicking .btn-primary", () => {
		document.body.innerHTML = `
    ...
    `;
		...
		const btn = document.querySelector("a") as HTMLElement;
	...
	it("fires CTA click event when clicking .btn-secondary", () => {
		document.body.innerHTML = `
      <div>
	...
	it("includes correct href, section, and page path", () => {
		document.body.innerHTML = `
	...
	it("does not include raw button text in CTA analytics", () => {
