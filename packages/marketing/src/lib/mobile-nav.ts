...
export function initMobileNav(details: HTMLElement): Cleanup {
	...
	const closeButton = details.querySelector("[data-mobile-nav-close]") as HTMLButtonElement | null;
	...
	closeButton?.addEventListener("click", handleBackdropPress);
	...
	return () => {
		...
		closeButton?.removeEventListener("click", handleBackdropPress);
		actionLinks.forEach((link) => {
