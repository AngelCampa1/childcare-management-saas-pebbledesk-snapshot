...
function mockFetch(overrides?: { referralCode?: string; position?: number; surveyToken?: string }) {
...
describe("EmailCapture → PostSignupSurvey → ReferralShare full flow", () => {
	...
	it("answering all survey questions calls fetch /api/survey with correct payload", async () => {
		...
		await waitFor(() => {
			...
			const body = JSON.parse((surveyCalls[0][1] as RequestInit).body as string) as Record<
				string,
				unknown
			>;
	...
	it("ReferralShare appears with correct referralUrl after survey completion", async () => {
		...
		await waitFor(() => {
			expect(screen.getByDisplayValue("https://testapp.com/?ref=REFCODE1")).toBeDefined();
		});
		expect(screen.getByText("You're signup #5")).toBeDefined();
	});

	it("referralCode and position from API thread through to ReferralShare correctly", async () => {
		vi.useFakeTimers({ shouldAdvanceTime: true });
		vi.stubGlobal("fetch", mockFetch({ referralCode: "TESTREF", position: 42 }));

		render(<EmailCapture {...defaultProps} productDomain="testapp.com" productName="TestApp" />);
		await submitEmail();
		await advanceToSurvey();
		await completeSurvey();

		await waitFor(() => {
			expect(screen.getByDisplayValue("https://testapp.com/?ref=TESTREF")).toBeDefined();
		});
