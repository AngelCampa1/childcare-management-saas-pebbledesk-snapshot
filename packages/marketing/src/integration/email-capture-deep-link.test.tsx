 ...
 * ?survey=open&e=<base64-email>&t=<token>, the EmailCapture component:
 *   1. Decodes the email from the `e` param
 *   2. Sets surveyToken from the `t` param
 *   3. Opens PostSignupSurvey immediately (no 1.5s delay)
 *   4. Threads the surveyToken through to the /api/survey fetch call
 */
...
describe("EmailCapture deep link flow (?survey=open)", () => {
	it("deep link auto-opens survey without 1.5s delay", () => {
		...
		Object.defineProperty(window, "location", {
			value: {
				search: `?survey=open&e=${btoa("user@example.com")}&t=deep-link-token`,
			},
			writable: true,
			configurable: true,
		});
	...
	it("surveyToken from URL ?t= param threads through to /api/survey fetch call", async () => {
		...
		Object.defineProperty(window, "location", {
			value: {
				search: `?survey=open&e=${btoa("user@example.com")}&t=deep-link-token-xyz`,
			},
			writable: true,
			configurable: true,
		});
		...
		const [, opts] = fetchMock.mock.calls.find(
			(call: unknown[]) =>
				typeof call[0] === "string" && (call[0] as string).includes("/api/survey"),
	...
	it("answers array from survey is correctly formed in the fetch payload", async () => {
		...
		Object.defineProperty(window, "location", {
			value: {
				search: `?survey=open&e=${btoa("user@example.com")}&t=tok-abc`,
			},
			writable: true,
			configurable: true,
		});
		...
		await waitFor(() => {
			expect(fetchMock).toHaveBeenCalledWith("https://api.test/api/survey", expect.any(Object));
		});
	...
	it("does not auto-open survey when e param is missing", () => {
		vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response("ok")));
		...
		// No email decoded → stays on the email form
		expect(screen.queryByText("What is your role?")).toBeNull();
		expect(screen.getByRole("button", { name: "Continue" })).toBeDefined();
	});

	it("ignores malformed base64 in e param", () => {
		vi.stubGlobal("fetch", vi.fn().mockResolvedValue(new Response("ok")));
