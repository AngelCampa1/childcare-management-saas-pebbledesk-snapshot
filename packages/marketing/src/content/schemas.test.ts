...
describe("baseContentSchema", () => {
	...
	it("accepts canonicalHref, redirectFrom, lastReviewed, and summaryFacts", () => {
		const result = baseContentSchema.parse({
			...validBase,
			canonicalHref: "/resources/guides/childcare-billing-guide",
			redirectFrom: ["/resources/guides/old-billing-guide"],
			lastReviewed: "2026-04-10",
			summaryFacts: [
				"Licensed centers can lose revenue when subsidy attendance and claims do not match.",
				"PebbleDesk keeps ratio, attendance, and billing support in one record.",
			],
		});

		expect(result.canonicalHref).toBe("/resources/guides/childcare-billing-guide");
		expect(result.redirectFrom).toEqual(["/resources/guides/old-billing-guide"]);
		expect(result.lastReviewed).toBe("2026-04-10");
		expect(result.summaryFacts).toHaveLength(2);
	});

	it("defaults redirectFrom to an empty array and summaryFacts to an empty array", () => {
		const result = baseContentSchema.parse(validBase);
		expect(result.redirectFrom).toEqual([]);
...
describe("leadMagnetSchema", () => {
	...
	it("accepts canonicalHref, redirectFrom, lastReviewed, and summaryFacts on lead magnets", () => {
		const result = leadMagnetSchema.parse({
			...validLeadMagnet,
			canonicalHref: "/free/licensing-compliance-checklist",
			redirectFrom: ["/free/daycare-licensing-checklist"],
			lastReviewed: "2026-04-10",
			summaryFacts: [
				"Licensing prep works better when attendance, ratios, and audit records stay together.",
			],
		});

		expect(result.canonicalHref).toBe("/free/licensing-compliance-checklist");
		expect(result.redirectFrom).toEqual(["/free/daycare-licensing-checklist"]);
		expect(result.lastReviewed).toBe("2026-04-10");
		expect(result.summaryFacts).toEqual([
			"Licensing prep works better when attendance, ratios, and audit records stay together.",
		]);
