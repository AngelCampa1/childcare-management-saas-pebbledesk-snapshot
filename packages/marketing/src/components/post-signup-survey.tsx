	...
	return (
		<div className="fixed inset-0 bg-[var(--surface-overlay)] backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
			<div
				...
				tabIndex={-1}
				className="bg-[var(--surface-elevated)] rounded-[var(--radius-lg)] shadow-[var(--shadow-xl)] max-w-md w-full text-center overflow-hidden animate-[var(--animate-scale-in)] focus:outline-none"
			>
				...
				<div className="p-8 relative">
					<button
						...
						onClick={onComplete}
						aria-label="Close"
						className="absolute top-4 right-4 w-11 h-11 flex items-center justify-center rounded-full text-[var(--color-neutral-400)] hover:text-[var(--color-brand-text)] hover:bg-[var(--surface-secondary)] transition-colors"
					>
					...
					{body && <p className="text-[var(--color-brand-muted)] mb-6">{body}</p>}
					{primaryCta && (
						<a
					...
					{dismissText && (
						<button type="button" onClick={onComplete} className="btn-secondary block mx-auto mt-4">
							{dismissText}
						</button>
					)}
					{referralSection}
...
export function PostSignupSurvey({
	...
	async function selectAnswer(questionId: string, answer: string) {
		...
		trackEvent(ANALYTICS_EVENTS.surveyCompleted, {
		...
		return (
			<div className="fixed inset-0 bg-[var(--surface-overlay)] backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
				<div
					...
					tabIndex={-1}
					className="bg-[var(--surface-elevated)] rounded-[var(--radius-lg)] shadow-[var(--shadow-xl)] max-w-md w-full text-center overflow-hidden focus:outline-none"
				>
					<div className="p-8">
						<h3
							className="font-heading font-bold mb-3"
						...
						<button type="button" onClick={retrySubmission} className="btn-primary btn-shimmer">
							Try Again
						</button>
						<button type="button" onClick={onComplete} className="btn-secondary block mx-auto mt-4">
							Skip
	...
	if (status === "done") {
		...
		const referralUrl =
			productDomain && referralCode ? `https://${productDomain}/?ref=${referralCode}` : "";

		const referralSection =
			referralCode && productName && productDomain && position !== undefined && position > 0 ? (
	...
	return (
		<div className="fixed inset-0 bg-[var(--surface-overlay)] backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
			<div
				...
				tabIndex={-1}
				className="bg-[var(--surface-elevated)] rounded-[var(--radius-lg)] shadow-[var(--shadow-xl)] max-w-md w-full overflow-hidden animate-[var(--animate-scale-in)] focus:outline-none"
			>
				<div className="w-full h-1 bg-[var(--color-neutral-200)]">
					<div
				...
				<div className="p-8 relative">
					<button
						...
						className="transition-colors absolute top-4 right-4 w-11 h-11 flex items-center justify-center rounded-full text-[var(--color-neutral-400)] hover:text-[var(--color-brand-text)] hover:bg-[var(--surface-secondary)]"
					>
						<svg width="14" height="14" viewBox="0 0 14 14" fill="none" aria-hidden="true">
							<path
								d="M1 1l12 12M13 1L1 13"
					...
					<h3
						className="font-heading font-bold mb-6 pr-8 text-[var(--color-brand-text)]"
						style={{
							fontSize: "var(--text-heading)",
						}}
					>
					...
					<div className="space-y-3">
						{question.options.map((option) => (
							<button
								...
								className={clsx(
									"transition-colors",
									"w-full text-left px-4 py-3.5 rounded-[var(--radius-md)]",
									"bg-[var(--surface-secondary)] border border-[var(--color-neutral-200)]",
									"text-[var(--color-brand-text)] text-[length:var(--text-caption)]",
