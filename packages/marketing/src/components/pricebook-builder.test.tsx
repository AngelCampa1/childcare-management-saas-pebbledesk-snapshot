...
describe("PricebookBuilder", () => {
	...
	describe("trade selector", () => {
		...
		it("calls trackEvent when trade changes", () => {
			...
			expect(trackEvent).toHaveBeenCalledWith(
				ANALYTICS_EVENTS.pricebookBuilderInputsChanged,
	...
	describe("email gate - submission", () => {
		...
		it("shows CTA after success", async () => {
			...
			await waitFor(() => {
				expect(screen.getByText(/start your 30-day free trial/i)).toBeDefined();
				expect(screen.getByRole("link", { name: /start your 30-day free trial/i })).toBeDefined();
			});
		});
		...
		it("calls trackEvent with pricebook PDF request event on success", async () => {
			vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true }));
			...
			await waitFor(() => {
				expect(trackEvent).toHaveBeenCalledWith(
					ANALYTICS_EVENTS.pricebookPdfRequested,
