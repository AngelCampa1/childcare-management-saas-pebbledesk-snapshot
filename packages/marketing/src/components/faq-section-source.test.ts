...
describe("faq section source regressions", () => {
	...
	it("emits FAQ schema by default when pages render the component", () => {
		const source = readSource("./faq-section.astro");

		expect(source).toContain("emitSchema = true");
		expect(source).toContain("buildFaqPageSchema");
		expect(source).toContain("<SchemaMarkup schemas={faqSchema ? [faqSchema] : []} />");
	});
});
