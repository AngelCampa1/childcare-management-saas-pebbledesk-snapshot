...
describe("EmailCapture", () => {
	...
	it("renders privacy note when provided", () => {
		render(
			<EmailCapture {...defaultProps} privacyNote="We respect your inbox. Unsubscribe anytime." />,
		);
		expect(screen.getByText("We respect your inbox. Unsubscribe anytime.")).toBeDefined();
	});
	...
	it("does not render privacy note when set to undefined", () => {
		...
		expect(screen.queryByText("We respect your inbox. Unsubscribe anytime.")).toBeNull();
	});
	...
	it("shows default errorInvalidEmail when error prop is not provided", () => {
		...
		const { errorInvalidEmail: _a, errorGeneric: _c, ...propsWithoutErrors } = defaultProps;
	...
	it("uses persisted attribution when the current page query string is empty", async () => {
		...
		await waitFor(() => {
			expect(vi.mocked(trackEvent)).toHaveBeenCalledWith(ANALYTICS_EVENTS.signupCompleted, {
	...
	it("parses referralCode and position from 409 response body", async () => {
		...
		await waitFor(() => {
			expect(screen.getByText("You're signup #7")).toBeDefined();
		});
		expect(screen.getByDisplayValue("https://test.app/?ref=ret123")).toBeDefined();
	});
	...
	it("does not render privacy note when privacyNote prop is not provided", () => {
		const { privacyNote: _, ...propsWithoutPrivacy } = defaultProps;
		render(<EmailCapture {...propsWithoutPrivacy} />);
		expect(screen.queryByText("We'll never spam you. Unsubscribe anytime.")).toBeNull();
	});
	...
	it("passes referral props to PostSignupSurvey after successful signup with referralCode in response", async () => {
		...
		expect(screen.getByDisplayValue("https://test.app/?ref=abc123")).toBeDefined();
	});
	...
	it("keeps the loading state visible when survey=open but the signup flow config is still loading", () => {
		const encodedEmail = btoa("autoopen@example.com");
		...
		const {
			...
			discoveryCallUrl: _discoveryCallUrl,
			...remoteConfigProps
		} = defaultProps;
		...
		render(
			<EmailCapture
				{...remoteConfigProps}
				signupFlowConfigUrl="https://marketing.test/signup-flow.json"
			/>,
		);
	...
	it("shows the signup-flow retry state when survey=open and config loading fails", async () => {
		const encodedEmail = btoa("autoopen@example.com");
	...
	it("does not pass referral data when API response has no referralCode", async () => {
		...
		render(
			<EmailCapture
				{...defaultProps}
				referralRewards={[{ threshold: 3, description: "Get 7 extra trial days" }]}
				productName="TestProduct"
				productDomain="test.app"
			/>,
		);
	...
	describe("analytics event tracking", () => {
		...
		it("fires signup completed event with correct properties on successful signup", async () => {
			vi.useFakeTimers({ shouldAdvanceTime: true });
			vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true, json: async () => ({}) }));
			Object.defineProperty(window, "location", {
				value: {
					search: "?utm_source=google&utm_medium=cpc&utm_campaign=spring&ref=abc",
				},
				writable: true,
				configurable: true,
			});
		...
		it("fires signup completed event without UTM keys when no query params", async () => {
			vi.useFakeTimers({ shouldAdvanceTime: true });
			vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true, json: async () => ({}) }));
			...
			await waitFor(() => {
				...
				expect(eventName).toBe(ANALYTICS_EVENTS.signupCompleted);
		...
		it("fires signup_duplicate with source_page when errorDuplicate prop is set and API returns 409", async () => {
			...
			await waitFor(() => {
				expect(vi.mocked(trackEvent)).toHaveBeenCalledWith(ANALYTICS_EVENTS.signupDuplicate, {
					source_page: "/pricing",
				});
			});
		});
		...
		it("fires signup submitted event alongside signup completed event with source=email_capture and UTM props", async () => {
			vi.useFakeTimers({ shouldAdvanceTime: true });
			vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true, json: async () => ({}) }));
			Object.defineProperty(window, "location", {
				value: {
			...
			await waitFor(() => {
				expect(vi.mocked(trackEvent)).toHaveBeenCalledWith(ANALYTICS_EVENTS.signupSubmitted, {
		...
		it("fires signup submitted event without UTM keys when no query params", async () => {
			vi.useFakeTimers({ shouldAdvanceTime: true });
			vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: true, json: async () => ({}) }));
			...
			await waitFor(() => {
				const submittedCall = vi
					.mocked(trackEvent)
					.mock.calls.find(([name]) => name === ANALYTICS_EVENTS.signupSubmitted);
				expect(submittedCall).toBeDefined();
				const [, props] = submittedCall as NonNullable<typeof submittedCall>;
				expect(props).toMatchObject({
					source: "email_capture",
					source_page: "/home",
				});
		...
		it("does NOT fire signup submitted event on error responses", async () => {
			vi.stubGlobal("fetch", vi.fn().mockResolvedValue({ ok: false, status: 500 }));
	...
	it("only one timer is pending when a duplicate submission immediately follows a 200 signup", async () => {
		...
		const form = document.querySelector("form[aria-label]") as HTMLElement;
		await act(async () => {
