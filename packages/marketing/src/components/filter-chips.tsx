...
import type { ContentItem, FilterDef, SortOption } from "../types";
...
export function FilterChips({
	...
	// biome-ignore lint/correctness/useExhaustiveDependencies: `items` is intentionally in deps to re-sync when the parent passes a new data array; refs stabilize callback/filter identities
	useEffect(() => {
	...
	return (
		...
		>
			{filters.map((filter) => {
				return (
					// biome-ignore lint/a11y/useSemanticElements: fieldset/legend would require CSS reset to preserve flex layout; role="group" + aria-label is equivalent and valid
					<div
						key={filter.id}
						role="group"
			...
			{sortOptions && sortOptions.length > 1 && (
				<div className="flex items-center gap-1.5 ml-auto">
					...
					<select
						...
						className="border border-[var(--color-neutral-200)] rounded px-2 py-1 text-[var(--color-brand-text)] bg-[var(--surface-primary)]"
						style={{ fontSize: "var(--text-caption)" }}
					>
						{sortOptions.map((opt) => (
