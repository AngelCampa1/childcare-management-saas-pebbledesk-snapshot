...
describe("ReferralShare", () => {
	...
	afterEach(() => {
		vi.useRealTimers();
		vi.clearAllMocks();
		document.querySelectorAll("[data-referral-url]").forEach((el) => {
			el.remove();
		});
	});
	...
	it("renders Share on X link with correct href", () => {
		render(<ReferralShare {...defaultProps} />);
		const link = screen.getByText("Share on X").closest("a") as HTMLAnchorElement;
	...
	it("renders Share on LinkedIn link with correct href", () => {
		render(<ReferralShare {...defaultProps} />);
		const link = screen.getByText("Share on LinkedIn").closest("a") as HTMLAnchorElement;
	...
	it("input is read-only", () => {
		render(<ReferralShare {...defaultProps} />);
		const input = screen.getByDisplayValue("https://example.com/ref/abc123") as HTMLInputElement;
	...
	it("fires referral link copied event when user copies their referral link", async () => {
		...
		expect(trackEvent).toHaveBeenCalledWith(ANALYTICS_EVENTS.referralLinkCopied);
	});
});
