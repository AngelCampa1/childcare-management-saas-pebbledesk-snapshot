...
export function PricebookBuilder({
	...
	function handleTradeChange(newTrade: string) {
		setTrade(newTrade);
		trackEvent(ANALYTICS_EVENTS.pricebookBuilderInputsChanged, {
			trade: newTrade,
	...
	async function handleDownloadSubmit(e: React.FormEvent) {
		...
		try {
			...
			if (res.ok) {
				trackEvent(ANALYTICS_EVENTS.pricebookPdfRequested, {
					trade,
					email_provided: true,
				});
			...
			>
					...
					<div style={{ display: "flex", gap: "var(--component-gap-sm, 0.75rem)" }}>
						{trades.map((tradeOption) => (
							<button
								...
								className={clsx(
									"px-4 py-2 rounded-[var(--radius-md)] border font-medium transition-colors",
				...
				<div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
					...
					<input
						...
						className="px-4 py-2 rounded-[var(--radius-md)] border border-[var(--color-neutral-300)] bg-[var(--surface-primary)] focus:outline-none focus:border-[var(--color-brand-primary)]"
			...
			>
				{downloadStatus === "success" ? (
					...
					>
						...
						<p className="text-[var(--color-brand-muted)]" style={{ fontSize: "var(--text-body)" }}>
