...
interface ReferralShareProps {
	...
	productName: string;
	/** Use #{position} as placeholder. Example: "You're signup ##{position}" renders as "You're signup #42" */
...
export function ReferralShare({
	...
	rewards,
	productName,
	positionLabel = "You're signup ##{position}",
	...
	async function copyToClipboard() {
		try {
			...
			setCopied(true);
			trackEvent(ANALYTICS_EVENTS.referralLinkCopied);
		...
		<div className="text-left">
			...
			<div className="flex gap-2 mb-3">
				<input
					...
					value={referralUrl}
					className="font-mono flex-1 px-3 py-2 rounded-[var(--radius-md)] border border-[var(--color-neutral-200)] bg-[var(--surface-sunken)] text-[var(--color-brand-text)] truncate"
					style={{
						fontSize: "var(--text-caption)",
					}}
				/>
				<button
					...
					className={[
						copied
							? "btn-shimmer min-w-[70px] rounded-full border border-[var(--color-success-300)] bg-[var(--color-success-100)] px-3 py-2 font-semibold text-[var(--color-success-700)] transition-colors"
							: "btn-primary btn-shimmer px-3 py-2 min-w-[70px]",
					].join(" ")}
					style={{ fontSize: "var(--text-caption)" }}
				>
			...
			<div className="flex gap-2 mb-3">
				<a
					...
					className="flex flex-1 items-center justify-center gap-1.5 rounded-full border border-[var(--color-neutral-200)] px-3 py-2 font-medium text-[length:var(--text-caption)] text-[var(--color-brand-text)] transition-colors hover:bg-[var(--surface-secondary)]"
				>
