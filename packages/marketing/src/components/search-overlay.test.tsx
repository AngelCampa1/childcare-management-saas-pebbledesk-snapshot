...
import { loadPagefindModule, SearchOverlay } from "./search-overlay";
...
describe("SearchOverlay", () => {
	...
	const getResultLinks = (container: HTMLElement) =>
		Array.from(container.querySelectorAll("#search-results a")) as HTMLAnchorElement[];
	const getResultItems = (container: HTMLElement) =>
		Array.from(container.querySelectorAll("#search-results li")) as HTMLLIElement[];
	...
	it("ArrowDown moves focus to the first result", async () => {
		...
		// Press ArrowDown to focus first result — the dialog panel itself is .max-w-lg
		const panel = container.querySelector("[role='dialog']") as HTMLElement;
		fireEvent.keyDown(panel, {
			key: "ArrowDown",
		});

		const resultLinks = getResultLinks(container);
		const resultItems = getResultItems(container);
		expect(document.activeElement).toBe(resultLinks[0]);
		expect(resultItems[0].getAttribute("data-active")).toBe("true");
		expect(resultItems[1].getAttribute("data-active")).toBeNull();
	});
	...
	it("ArrowUp from first result wraps to last result", async () => {
		...
		const searchPanel = container.querySelector(".max-w-lg") as HTMLElement;
		...
		const resultLinks = getResultLinks(container);
		const resultItems = getResultItems(container);
		expect(document.activeElement).toBe(resultLinks[1]);
		expect(resultItems[1].getAttribute("data-active")).toBe("true");
	});
	...
	it("activating the focused result link closes the overlay", async () => {
		const mockResult = {
			url: "/enter-page",
		...
		const [focusedLink] = getResultLinks(container);
	...
	it("Enter with results but no active item (activeIndex -1) does nothing", async () => {
		...
		const listItems = getResultItems(container);
	...
	it("active index resets when query changes", async () => {
		...
		let resultItems = getResultItems(container);
		expect(resultItems[0].getAttribute("data-active")).toBe("true");
		...
		resultItems = getResultItems(container);
		expect(resultItems[0].getAttribute("data-active")).toBeNull();
	});
	...
	it("closes overlay when a result item is clicked", async () => {
		...
		const [link] = getResultLinks(container);
	...
	it("results list exposes a labeled list with id=search-results", async () => {
		const mockResult = {
			url: "/aria-page",
	...
	it("each result renders as a plain list item containing a link", async () => {
		const mockResults = [
			{ url: "/page-1", meta: { title: "Page One" }, excerpt: "First" },
			{ url: "/page-2", meta: { title: "Page Two" }, excerpt: "Second" },
		];
	...
	it("clicking a result link closes the overlay", async () => {
		const mockResult = {
	...
	it("pressing Space on a result link does not close the overlay", async () => {
		const mockResult = {
	...
	it("other keys on result links do not navigate or close the overlay", async () => {
		const mockResult = {
			url: "/other-key-page",
	...
	it("results render as links without nested buttons", async () => {
		const mockResults = [
			{ url: "/page-1", meta: { title: "Page One" }, excerpt: "First" },
			{ url: "/page-2", meta: { title: "Page Two" }, excerpt: "Second" },
		];
	...
	it("input does not set aria-activedescendant for link-based results", async () => {
		const mockResult = {
			url: "/desc-page",
	...
	it("ArrowDown advances focus across result links", async () => {
		const mockResults = [
			{ url: "/page-1", meta: { title: "Page One" }, excerpt: "First" },
			{ url: "/page-2", meta: { title: "Page Two" }, excerpt: "Second" },
		];
	...
	it("active result item is tracked with data-active", async () => {
		const mockResults = [
			{ url: "/page-1", meta: { title: "Page One" }, excerpt: "First" },
			{ url: "/page-2", meta: { title: "Page Two" }, excerpt: "Second" },
		];
	...
	it("pressing Enter on the search panel does not close the overlay", async () => {
		const mockResults = [
			{ url: "/page-1", meta: { title: "Page One" }, excerpt: "First" },
			{ url: "/page-2", meta: { title: "Page Two" }, excerpt: "Second" },
		];
	...
	it("only the active result has tabIndex=0, others have tabIndex=-1", async () => {
		...
		listItems.forEach((li) => {
			...
			expect(li.querySelector("a")?.hasAttribute("tabindex")).toBe(false);
		});
	});

	// --- Native search input semantics ---

	it("input stays a native search field instead of declaring combobox semantics", async () => {
		const mockResult = {
			url: "/combobox-page",
